...
describe("session buffer", () => {
  ...
  it("records lines in order", () => {
    recordLine({ id: "a", text: "hello", receivedAt: T + 1_000 });
    recordLine({ id: "b", text: "world", receivedAt: T + 2_000 });
    const snap = sessionSnapshot();
    expect(snap.lines).toEqual([
      { ts: T + 1_000, text: "hello" },
      { ts: T + 2_000, text: "world" },
    ]);
...
describe("flushSession", () => {
  ...
  it("returns null and writes nothing when saving is disabled", () => {
    ...
    settingsRef.dir = dir;
    recordLine({ id: "a", text: "hello", receivedAt: T });
    expect(flushSession()).toBeNull();
    expect(readdirSync(dir)).toHaveLength(0);
  ...
  it("writes a timestamped .txt when enabled and clears the session", () => {
    ...
    settingsRef.enabled = true;
    settingsRef.dir = dir;
    recordLine({ id: "a", text: "first", receivedAt: T });
    recordLine({ id: "b", text: "second", receivedAt: T + 5_000 });
    ...
    expect(body).toContain("[14:07:09] first");
    expect(body).toContain("[14:07:14] second");
  ...
  it("creates the destination directory if missing", () => {
    ...
    settingsRef.enabled = true;
    settingsRef.dir = dir;
    recordLine({ id: "a", text: "hi", receivedAt: T });
    const file = flushSession();
    expect(file).not.toBeNull();
